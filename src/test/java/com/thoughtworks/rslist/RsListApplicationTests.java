...
public  class RsListApplicationTests {
    ...
    public void should_modify_one_rs_events_by_index() throws Exception {
        ...
        mockMvc.perform(patch("/rs/{1}").content(newJson).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
        mockMvc.perform(get("/rs/list"))
                .andExpect(jsonPath("$",hasSize(1)))
                .andExpect(jsonPath("$[0].eventName", is("修改事件")))
                .andExpect(jsonPath("$[0].keyWord", is("暂无")));
    }
